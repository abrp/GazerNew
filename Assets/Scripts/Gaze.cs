...
public class Gaze : MonoBehaviour {
	...
	private void EyeRaycast(){
		...
		if (Physics.Raycast(startPosition,direction * m_rayLenght, out hit,m_rayLenght)){
			...
			//Debug.Log (hit.transform.name + " at position " + hit.point);

			if (Input.GetMouseButtonDown(0)) {
				//transform.position = new Vector3 (m_reticle.transform.position.x, 2.5f, m_reticle.transform.position.z);
				if(hit.transform.tag == "MoveArea") {
